...
{
    public class BreederStandardAdvancedFeedingConfiguration : IControllerConfiguration
    {
        private Dictionary<Type, ControllerPageSetup> _fixedPagesSetupDictionary = new Dictionary<Type, ControllerPageSetup>()
        {
        #region Breeder Standard Advanced Feeding pages
    ...
    PageTitles = new List<string>() { "Foggers" },
            ...
            #region Feed Scale Program
                ...
                typeof(FeedScaleProgramPage), new ControllerPageSetup()
                {
                    ...
                    TableColumns = new List<ITableColumn>()
                        {
                            new TextColumn(){ DisplayName = "No", InputMethod = new DateKeyPad(), IsReadOnly = true},
                            new TextColumn(){ DisplayName = "Start Time", InputMethod = new TimeKeyPad(), IsReadOnly = false},
                            new TextColumn(){ DisplayName = "Type", InputMethod = new TouchComboBox(), IsReadOnly = false},
                            new TextColumn(){ Group ="Silo Augers(in %)", DisplayName = "1N", InputMethod = new BaseNumericKeyPad(), IsReadOnly = true},
                            new TextColumn(){ Group ="Silo Augers(in %)", DisplayName = "2N", InputMethod = new BaseNumericKeyPad(), IsReadOnly = true},
                            new TextColumn(){ Group ="Silo Augers(in %)", DisplayName = "3N", InputMethod = new BaseNumericKeyPad(), IsReadOnly = true},
                            new TextColumn(){ Group ="Silo Augers(in %)", DisplayName = "4N", InputMethod = new BaseNumericKeyPad(), IsReadOnly = true},
                            new TextColumn(){ Group ="Silo Augers(in %)", DisplayName = "5N", InputMethod = new BaseNumericKeyPad(), IsReadOnly = true},
                            new TextColumn(){ Group ="Silo Augers(in %)", DisplayName = "6N", InputMethod = new BaseNumericKeyPad(), IsReadOnly = true},
                            new TextColumn(){ Group ="Silo Augers(in %)", DisplayName = "7N", InputMethod = new BaseNumericKeyPad(), IsReadOnly = true},
                            new TextColumn(){ Group ="Silo Augers(in %)", DisplayName = "8N", InputMethod = new BaseNumericKeyPad(), IsReadOnly = true},
                            new TextColumn(){ DisplayName = "Inter. Hopper Auger", InputMethod = new DateKeyPad(), IsReadOnly = true},
                    },
                    SettingsPageTitles = new List<string>(){"Feed Scale - Settings"},
                }
            }, 
	#endregion
                ...
                typeof(FoggersPage), new ControllerPageSetup()
{
    PageTitles = new List<string>() { "Foggers" },
        ...
        public BreederStandardAdvancedFeedingConfiguration()
        {
            Name = "BreederStandardAdvancedFeeding";
            WaterDevices = 4;
            FeedDevices = 4;
            AugerDevices = 4;
            LightDevices = 1;
            ...
            {
                ControllerType = ControllerType.PLATINUM_PLUS_BREEDER_STANDARD,
                MemoryVersion = 8,
                ControllerMajorVersion = 5,
                LevelControl = LevelControl.CM_SoftMin,

                /* Online path:*/
                MetadataDirectoryInfo = new DirectoryInfo(Path.GetFullPath("../../../Rotem.Touch.UITests.Infrastructure/ExternalDependencies//PLATINUM_PLUS_BREEDER_STANDARD_METADATA/"))
