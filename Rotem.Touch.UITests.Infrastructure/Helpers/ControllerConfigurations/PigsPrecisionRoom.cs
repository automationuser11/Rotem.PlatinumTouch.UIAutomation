...
{
    public class PigsPrecisionRoomConfiguration : IControllerConfiguration
    {
...
#region RadiantHeatersHistory
        ...
        private Dictionary<Type, ControllerPageSetup> _fixedPagesSetupDictionary = new Dictionary<Type, ControllerPageSetup>()
        {
            ...
            #region Page Name: Temperature Curve
		   {
            ...
            #region PAGE name: Animal Inventory
            {
                typeof(AnimalInventoryPage), new ControllerPageSetup()
                {
                    PageTitles = new List<string>() { "Animal Inventory" },
                    TableColumns = new List<ITableColumn>()
                        {
                            new TextWithImageColumn(){ DisplayName = "", IsReadOnly = true },
                            new TextColumn(){ DisplayName = "Animals Placed", InputMethod = new BaseNumericKeyPad(), IsReadOnly = false },
                            new TextColumn(){ Group = "Dead Animals", DisplayName = "Update", InputMethod = new MinusNumericKeyPad(), IsReadOnly = false },
                            new TextColumn(){ Group = "Dead Animals", DisplayName = "Total", IsReadOnly = true },
                            new TextColumn(){ Group = "Culled Animals", DisplayName = "Update", InputMethod = new MinusNumericKeyPad(), IsReadOnly = false },
                            new TextColumn(){ Group = "Culled Animals", DisplayName = "Total", IsReadOnly = true },
                            new TextColumn(){ Group = "Animals Moved", DisplayName = "Update", InputMethod = new MinusNumericKeyPad(), IsReadOnly = false },
                            new TextColumn(){ Group = "Animals Moved", DisplayName = "Total", IsReadOnly = true },
                            new TextColumn(){ DisplayName = "Animals Count", IsReadOnly = true },
                        }
                }
            }, 
            #endregion

            #region PAGE name: Growth Day & Group
            {
                typeof(GrowthDayAndGroupPage), new ControllerPageSetup()
                {
                    PageTitles = new List<string>(){ "Growth Day & Group" },
                }
            }, 
            #endregion
            ...
            #region PAGE name: Silo Settings 
            {
                typeof(SiloSettingsPage), new ControllerPageSetup()
                {
                    PageTitles = new List<string>() { "Silo Settings" },
                }
            }, 
            #endregion
        ...
        public PigsPrecisionRoomConfiguration()
        {
            Name = "PigsPrecisionRoom";
            WaterDevices = 4;
            FeedDevices = 4;
            AugerDevices = 4;
            LightDevices = 1;
            ...
            {
                ControllerType = ControllerType.PLATINUM_PLUS_PIGS_PRECISION,
                MemoryVersion = 8,
                ControllerMajorVersion = 5,
                LevelControl = LevelControl.CM_SoftMin,

                /* Online path:*/
                MetadataDirectoryInfo = new DirectoryInfo(Path.GetFullPath("../../../Rotem.Touch.UITests.Infrastructure/ExternalDependencies//PLATINUM_PLUS_PIGS_PRECISION_METADATA/"))
